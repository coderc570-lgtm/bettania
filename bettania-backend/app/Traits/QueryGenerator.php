...
trait QueryGenerator
{
    ...
    public function paginateCustom($payload, $total, $model): array
    {
        $current_page = isset($payload['page']) ? (is_numeric($payload['page']) && $payload['page'] > 0) ? $payload['page'] : 1 : 1;
        $take = isset($payload['show']) ? (is_numeric($payload['show']) ? $payload['show'] : (($payload['show'] == 'all') ? $model->get()->count() : $total)) : 15;
        $skip = (is_numeric($take) ? $take : 0) * ((isset($payload['page']) && $payload['page'] > 1) ? ($payload['page'] - 1) : 0);
    ...
    public function getJsonResponseSignature(array $data): JsonResponse
    {
        $status = $data['status'] ?? 200;
        return response()->json([
            'message' => $data['message'] ?? null,
    ...
    //gryff custom index (beta)
    public function customIndex($payload, $selected_relation_columns_only = [], $headers = [], ?callable $extraSelects = null, ?callable $extraJoins = null): array
    {
        $excludedIds = $payload['excluded_id'] ?? null;

        $start_date = $payload['start_date'] ?? null;
        $end_date = $payload['end_date'] ?? null;
        $by_document_status = $payload['by_document_status'] ?? null;
        $search_resolution_type_id = $payload['search_resolution_type_id'] ?? null;
        $by_document_class = $payload['by_document_class'] ?? null;
        ...
        // Apply additional selects if provided
        if ($extraSelects) {
            $extraSelects($query);
        }

        // Apply additional joins if provided
        if ($extraJoins) {
            $extraJoins($query);
        }

        // Apply start_date and end_date filters if they exist
        if ($start_date) {
            $query->where($tableName . '.date_proposed', '>=', $start_date);
        }

        if ($end_date) {
            // Extend the end_date to include the full day
            $end_date = date('Y-m-d 23:59:59', strtotime($end_date));
            $query->where($tableName . '.date_proposed', '<=', $end_date);
        }

        if ($by_document_status) {
            $query->whereIn($tableName . '.document_status_id', $by_document_status);
        }

        if($search_resolution_type_id) {
            $query->whereIn($tableName . '.resolution_type_id', $search_resolution_type_id);
        }

        if ($by_document_class) {
            $query->whereIn($tableName . '.document_class_id', $by_document_class);
        }
    ...
    public function customSearchGlobal($payload, $query, $selects, $json_aggregates = [], $where = true): void
    {
        if (isset($payload['search_global']) && $payload['search_global']) {
            //            $dateTimeObject = \DateTime::createFromFormat('Y-m-d H:i:s', $payload['search_global']);
            //            if ( ctype_digit($payload['search_global']) || is_bool($payload['search_global']) ||
            ...
            if ($where) {
                $query->where(function ($query) use ($selects, $payload, $json_aggregates) {
                    $this->customGenerator($query, $selects, $payload, $json_aggregates);
                });

                $from_search_engine = $payload['from_search_engine'] ?? false;
                ...
                if ($from_search_engine) {
                    $query->orWhere(function ($q) use ($payload) {
                        $q->fullTextSearch(
                            ...
                            [
                                'resolution_histories.title',
                                'resolution_author_types.name',
                                'users.first_name',
                                'users.middle_name',
                                'users.last_name',
                            ],
                ...
                $query->orWhere(function ($query) use ($selects, $payload, $json_aggregates) {
                    $this->customGenerator($query, $selects, $payload, $json_aggregates);
                });
            }
        }
    ...
    public function customGenerator($query, $selects, $payload, $json_aggregates = [], $condition = 'orWhere'): void
    {
        $search_global = $payload['search_global'] ?? null;
        $from_transmittal = $payload['from_transmittal'] ?? false;

        if ($search_global  && !$from_transmittal) {
            $query->$condition(
                DB::raw("REGEXP_REPLACE(TRIM(CONCAT(users.first_name, ' ', COALESCE(users.middle_name, ''), ' ', users.last_name, COALESCE(users.suffix, ''))), '\\s+', ' ', 'g')"),
                'ilike',
                "%" . $search_global . "%"
            );
        }
        ...
        foreach ($json_aggregates as $aggregate) {
            // Ensure the aggregate has the required keys
            if (!isset($aggregate['json_field'], $aggregate['table_column'])) {
                continue; 
            }
    
            $jsonField = $aggregate['json_field'];
            $tableColumn = $aggregate['table_column']; 
    
            // Basic validation to prevent SQL injection
            if (!preg_match('/^[a-zA-Z0-9_]+\.[a-zA-Z0-9_]+$/', $tableColumn)) {
                continue; 
            }
    
            $query->$condition($tableColumn, 'ilike', "%{$search_global}%");
        }
    }

    public function searchGlobalWithName($payload, $query, $selects, $json_aggregates = [], $where = true): void
    {
        if (isset($payload['search_global']) && $payload['search_global']) {
            //            $dateTimeObject = \DateTime::createFromFormat('Y-m-d H:i:s', $payload['search_global']);
            //            if ( ctype_digit($payload['search_global']) || is_bool($payload['search_global']) ||
            ...
            if ($where) {
                $query->where(function ($query) use ($selects, $payload, $json_aggregates) {
                    $this->customGeneratorWithName($query, $selects, $payload, $json_aggregates);
                });
            } else {
                $query->orWhere(function ($query) use ($selects, $payload, $json_aggregates) {
                    $this->customGeneratorWithName($query, $selects, $payload, $json_aggregates);
    ...
    public function customGeneratorWithName($query, $selects, $payload, $json_aggregates = [], $condition = 'orWhere'): void
    {
        $search_global = $payload['search_global'] ?? null;
        ...
        if ($search_global) {
            ...
            foreach ($date_formats as $format) {
                $dateTimeObject = \DateTime::createFromFormat($format, $search_global);
                if ($dateTimeObject !== false && $dateTimeObject->format($format) === $search_global) {
                    $is_valid_date = true;
                    $converted_date = $dateTimeObject->format('Y-m-d 00:00:00');
                    break;
                }
            }
        
            // Fallback to strtotime for flexible parsing
            if (!$is_valid_date) {
                $timestamp = strtotime($search_global);
                if ($timestamp !== false) {
                    $is_valid_date = true;
                    $converted_date = date('Y-m-d 00:00:00', $timestamp);
                }
            }
        }
        ...
        foreach ($selects['columns'] as $column) {
            ...
            if ($type == 'timestamp') {
                ...
                if ($is_valid_date) {
                    $query->$condition($column['column'], '=', $converted_date);
                }
        ...
        foreach ($json_aggregates as $aggregate) {
            ...
            // Handle special case for author_name which uses CONCAT_WS
            if ($jsonField === 'author_name') {
                // Use whereExists to search within the author names
                $query->$condition(function($subQuery) use ($search_global, $payload) {
                    $subQuery->whereExists(function ($existsQuery) use ($search_global, $payload) {
                        $existsQuery->select(DB::raw(1))
                            ->from('resolution_authors')
                            ->leftJoin('users as resolution_author_user', 'resolution_author_user.id', '=', 'resolution_authors.user_id')
                            ->whereRaw('resolution_authors.resolution_id = resolutions.id')
                            ->whereRaw("UPPER(CONCAT_WS(' ', resolution_author_user.first_name, resolution_author_user.middle_name, resolution_author_user.last_name)) ILIKE ?", ["%".strtoupper($search_global)."%"])
                            ->whereNull('resolution_authors.deleted_at')
                            ->whereNull('resolution_author_user.deleted_at');
                        
                        // Add author type filter if specified
                        if (!empty($payload['search_by_author_type'])) {
                            $existsQuery->where('resolution_authors.resolution_author_type_id', $payload['search_by_author_type']);
                        }
                    });
                });
                continue;
            } else if ($jsonField === 'document_author_name') {
                // Use whereExists to search within the document author names
                $query->$condition(function($subQuery) use ($search_global, $payload) {
                    $subQuery->whereExists(function ($existsQuery) use ($search_global, $payload) {
                        $existsQuery->select(DB::raw(1))
                            ->from('document_authors')
                            ->leftJoin('users as document_author_user', 'document_author_user.id', '=', 'document_authors.user_id')
                            ->whereRaw('document_authors.document_id = documents.id')
                            ->whereRaw("UPPER(CONCAT_WS(' ', document_author_user.first_name, document_author_user.middle_name, document_author_user.last_name)) ILIKE ?", ["%".strtoupper($search_global)."%"])
                            ->whereNull('document_authors.deleted_at')
                            ->whereNull('document_author_user.deleted_at');
                        
                        // Add author type filter if specified
                        if (!empty($payload['search_by_document_author_type'])) {
                            $existsQuery->where('document_authors.document_author_type_id', $payload['search_by_document_author_type']);
                        }
                    });
                });
                continue;
            ...
            // Basic validation to prevent SQL injection for other fields
            if (!preg_match('/^[a-zA-Z0-9_]+\.[a-zA-Z0-9_]+$/', $tableColumn)) {
                continue; 
            }
    ...
    /**
     * Build dynamic JSON aggregation query
     * 
     * @param array $fields Key-value pairs for JSON object fields
     * @param string $filterColumn Column to filter on (e.g., 'publication_authors.id')
     * @param string $alias Alias for the aggregated column
     * @param bool $distinct Whether to use DISTINCT in aggregation
     * @param string $else Default value to return if no results (default: "'[]'::json")
     * @return string
     */
    protected function buildJsonAggregation(array $fields, string $filter_column, string $alias, bool $distinct = true, string $else = "'[]'::json"): string 
    {
        $json_fields = [];
        ...
        $json_field_string = implode(', ', $json_fields);
        $distinct_keyword = $distinct ? 'DISTINCT ' : '';
        
        return "COALESCE(
            JSON_AGG({$distinct_keyword}JSONB_BUILD_OBJECT(
                {$json_field_string}
            )) FILTER (WHERE {$filter_column} IS NOT NULL),
            {$else}
        ) as {$alias}";
    }
    
    protected function addJsonAggregation($query, array $fields, string $filter_column, string $alias, bool $distinct = true, string $else = "'[]'::json")
    {
        return $query->addSelect(DB::raw(
            $this->buildJsonAggregation($fields, $filter_column, $alias, $distinct, $else)
        ));
    }

    /**
     * Build single JSON object with conditional check
     * 
     * @param array $fields Key-value pairs for JSON object fields
     * @param string $filter_column Column to check for existence (e.g., 'file_locators.id')
     * @param string $alias Alias for the JSON object column
     * @param mixed $else_value Value to return when condition fails (default: null)
     * @return string
     */
    protected function buildJsonObject(array $fields, string $filter_column, string $alias, $else_value = 'NULL'): string
    {
        $json_fields = [];
        foreach ($fields as $key => $value) {
            $json_fields[] = "'{$key}', {$value}";
        ...
        return "CASE WHEN {$filter_column} IS NOT NULL THEN 
            JSONB_BUILD_OBJECT(
                {$json_field_string}
            )
        ELSE {$else_value} END as {$alias}";
    }

    protected function addJsonObject($query, array $fields, string $filter_column, string $alias, $else_value = 'NULL')
    {
        return $query->addSelect(DB::raw(
            $this->buildJsonObject($fields, $filter_column, $alias, $else_value)
        ));
    }

    /**
     * Generate a unique abbreviation for a given text
     * 
     * @param string $text The text to generate abbreviation from
     * @param array $existing_abbreviations Array of existing abbreviations to check against
     * @return string The unique abbreviation
     */
    private function generateUniqueAbbreviation($text, $existing_abbreviations = [])
    {
        $words = explode(' ', $text);
        $max_length = max(array_map('strlen', $words)); 
        
        for ($char_count = 1; $char_count <= $max_length; $char_count++) {
            $abbreviation = '';
            foreach ($words as $word) {
                // Only use alphanumeric first characters
                if (preg_match('/^[a-zA-Z0-9]/', $word)) {
                    $abbreviation .= strtoupper(substr($word, 0, min($char_count, strlen($word))));
                }
            }
            
            // If this abbreviation is unique, return it
            if (!in_array($abbreviation, $existing_abbreviations)) {
                return $abbreviation;
            }
        }

        if (count($words) > 1) {
            for ($focus_length = 2; $focus_length <= 4; $focus_length++) {
                for ($other_length = 1; $other_length <= 3; $other_length++) {
                    $abbreviation = '';
                    for ($i = 0; $i < count($words); $i++) {
                        $word = $words[$i];
                        if (preg_match('/^[a-zA-Z0-9]/', $word)) {
                            // Use longer abbreviation for first word, shorter for others, or vice versa
                            $length = ($i === 0) ? $focus_length : $other_length;
                            $abbreviation .= strtoupper(substr($word, 0, min($length, strlen($word))));
                        }
                    }
                    ...
                    // Try reverse: shorter for first word, longer for others
                    $abbreviation = '';
                    for ($i = 0; $i < count($words); $i++) {
                        $word = $words[$i];
                        if (preg_match('/^[a-zA-Z0-9]/', $word)) {
                            $length = ($i === 0) ? $other_length : $focus_length;
                            $abbreviation .= strtoupper(substr($word, 0, min($length, strlen($word))));
                        }
                    }
        ...
        // Fallback: if still not unique, append a number
        $base_abbreviation = $abbreviation;
        $counter = 1;
        while (in_array($abbreviation, $existing_abbreviations)) {
            $abbreviation = $base_abbreviation . $counter;
            $counter++;
        }
